...
const Charts = ({
  ...
  const [presetZoomLevel, setPresetZoom] = useState(10);
  const [zoomLevel, setZoomLevel] = useState(presetZoomLevel);
  ...
  const targetX = useRef<HTMLDivElement>(null);
  const targetY = useRef<HTMLDivElement>(null);

  let zoomStep = 4;

  useEffect(() => {
    console.log("Data Frame for ", title, values);
    for (var i in values) {
  ...
  // useEffect(() => {
  //   console.log("MinMax Y ::::", minY, maxY);
  // }, [minY, maxY]);
  ...
  const options: ApexOptions = {
    ...
    yaxis: {
      min: minY - zoomLevel * 0.0025, //-zoomLevel,
      max: maxY + zoomLevel * 0.0025, //zoomLevel,
      labels: {
    ...
    if (values.length) {
      ...
      if (values.length) {
        ...
        for (var i of values) {
          if (i[1] < min) min = i[1];
          if (i[1] > max) max = i[1];
          console.log("MinMax Y ::::", min, max, i[1]);
        }
        setMaxY(max);
        setMinY(min);
  ...
  useGesture(
    {
      ...
      onDrag: ({ event, offset: [, y], direction: [, dy] }: any) => {
        console.log("Drag", y);
        setLeftOffset(y);
        event.preventDefault();
        if (dy) {
  ...
  return (
    ...
    >
      ...
      >
        <div id="vertical-zoom-pane" onClick={() => console.log("clicked")} />{" "}
        <div id="horizontle-zoom-pane" onClick={() => console.log("clicked")} />
        <Chart
          type="area"
          options={options}
          series={dataFrame}
